...
export const CartDrawer = () => {
  ...
  const cartDescription = totalItems === 0
    ? t('cart.empty')
    : t(totalItems === 1 ? 'cart.itemCount_one' : 'cart.itemCount_other', { count: totalItems });
    ...
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      ...
      <SheetContent className="w-full sm:max-w-lg flex flex-col h-full bg-card border-border">
        <SheetHeader className="flex-shrink-0">
          <SheetTitle className="font-display text-foreground">{t('cart.title')}</SheetTitle>
                  ...
                  {items.map((item) => (
                    <div key={item.variantId} className="flex gap-4 p-3 bg-secondary/30 rounded-lg">
                      ...
                      <div className="flex-1 min-w-0">
                        <h4 className="font-semibold text-foreground truncate">{item.product.node.title}</h4>
